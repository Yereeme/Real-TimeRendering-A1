...
void Tutorial::ObjectsPipeline::create(RTG& rtg, VkRenderPass render_pass, uint32_t subpass) {
	...
	{ //the set0_Camera layout holds a Camera
		//structure in a uniform buffer used in the vertex shader:
		//the set1_Transforms layout holds an array of Transform structures in a storage buffer used
	...
	{//CREATE PIPELINE:
		...
		//depth and stencil tests disable:
		...
		//disable mulitisampling:(one sample per pixel):
		...
		//the rasterizer will cull back faces and fill polygon:
		...
		std::array < VkPipelineShaderStageCreateInfo, 2> stages{ //vertex shader pipeline mdule to run 
			...
			VkPipelineShaderStageCreateInfo{ //frag shader pipeline module to run
				.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO,
		...
		//all of the above structures get bundled together into one very large create_info and create 
		// actual pipeline:
		...
		//Null handle is a reference to an optional pipeline cache
