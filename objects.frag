#version 450
//for light,  normalize the interpolated vertex normal and then use the basic hemisphere 
//light equation to shade meshes:
...
void main() {
	...
	vec3 albedo = texture(TEXTURE, texCoord).rgb; //albedo = vec3(1.0) vec3(fract(texCoord), 0.0)
